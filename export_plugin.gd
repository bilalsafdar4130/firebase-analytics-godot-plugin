...
class FirebaseAndroidExportPlugin extends EditorExportPlugin:
	...
	##
	## THE AD-ID LINE IS THE LOUD ONE, and it stays DENIED even in a build that
	## serves ads. Firebase Analytics collects the Android advertising ID by
	## default, and that single default is what puts "Advertising ID" in a Play
	## Data Safety declaration and makes an ad-free build look like an ad build.
	## An ad SDK reading that identifier to fill a banner is one thing; ANALYTICS
	## also collecting it, and joining it to every gameplay event this app
	## reports, is a second and much wider collection that serving a banner does
	## not require. So the permission may come back (see `ADS_ADDON_PLUGIN_CFG`)
	## while this line does not move.
	const PRIVACY_META := {
	...
	## ...unless something in this project genuinely reads an advertising ID.
	##
	## The `admob_ads` addon serves AdMob, which does, and declares the
	## permission itself. Exactly one of the two addons may be right at a time,
	## and it is this one that stands down: an ads build that stripped the
	## permission would have its advertising identifier zeroed out and its ad
	## revenue with it, which is the more expensive of the two mistakes and the
	## harder one to notice.
	##
	## Read from the ENABLED-PLUGINS list rather than from the folder existing,
	## because an addon can sit in `addons/` switched off — and a disabled ads
	## addon means a build with no ads, which needs the removal after all.
	const ADS_ADDON_PLUGIN_CFG := "res://addons/admob_ads/plugin.cfg"
	...
	## Whether an ads addon in this project owns the advertising-ID permission.
	static func ads_addon_enabled() -> bool:
		var enabled = ProjectSettings.get_setting(
			"editor_plugins/enabled", PackedStringArray()
		)
		if enabled == null:
			return false
		for path in enabled:
			if str(path) == ADS_ADDON_PLUGIN_CFG:
				return true
		return false

	## The one-line removal of the advertising-ID permission the SDK brings in —
	## or nothing at all, when an ads addon needs it kept. See
	## `ADS_ADDON_PLUGIN_CFG`.
	##
	## Two functions rather than one with a default, so the branch the verifier
	## drives and the branch an export takes are the same code reached two ways:
	## this one answers from the project being exported, `ad_id_removal_xml_for`
	## answers a question put to it.
	static func ad_id_removal_xml() -> String:
		return ad_id_removal_xml_for(ads_addon_enabled())

	## The pure half — see above.
	static func ad_id_removal_xml_for(ads_addon_owns_it: bool) -> String:
		if ads_addon_owns_it:
			return "\n".join(PackedStringArray([
				"    <!-- The advertising-ID permission is declared by the admob_ads",
				"         addon: this build serves ads, which read it. -->",
				"",
			]))
		return "\n".join(PackedStringArray([
